<?php
    ...
    function fnGetCardProfile2($login) {
        ...
        $select = sprintf("SELECT `id_zayavka` FROM `sotrydnik` WHERE `login` = '%s'", $login);
        $select_result = $connect->query($select);
        $select_row = $select_result->fetch_assoc();
        $id = $select_row['id_zayavka'];

        

        $data = '<div class="container">';
        $sql = sprintf("SELECT `sotrydnik`.`id_zayavka`, `tip_nomera`, `zaezd`, `viezd`, `status` FROM
        `zayavka` join `sotrydnik` INNER JOIN `nomer` ON `zayavka`.`id_nomer` = `nomer`.`id_nomer`
        where `sotrydnik`.`id_zayavka` = `zayavka`.`id_zayavka` and `sotrydnik`.`id_zayavka` = '%s' ORDER BY `id_zayavka` DESC", $id);
